...
export class MediaController {
  static async upload(req: Request, res: Response, next: NextFunction): Promise<void> {
    try {
      ...
      // gracias al "declare global" de tu auth.middleware
      ...
      // Reemplazado <any> por <IMedia> para mantener el contrato estricto del Modelo
      const succeeded = settled
        .filter((r): r is PromiseFulfilledResult<IMedia> => r.status === 'fulfilled')
        .map((r) => r.value);
